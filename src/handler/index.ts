...
export default async (client: any) => {
  ...
  const buttonsFiles = readdirSync("./src/buttons").filter((file) => file.endsWith(".ts"));
  for (const file of buttonsFiles) {
    const button = await import(`../buttons/${file}`);
    await client.buttons.set(button.default.data.data.custom_id, button);
    console.log(`${button.default.data.data.custom_id} button loaded successfully.`);
  }
  ...
  for (const file of eventFiles) {
    ...
    if (event.once) {
      client.once(event.default.name, (...args: any) => event.default.run(...args, client));
    } else {
      client.on(event.default.name, async (...args: any) => await event.default.run(...args, client));
    }
  }

  client.on("ready", async () => {
    await client.guilds.cache.get(process.env.GUILD).commands.set(commandsArray);
